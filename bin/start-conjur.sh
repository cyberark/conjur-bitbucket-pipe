...
main() {
  ...
  admin_api_key=$(exec_on conjur conjurctl role retrieve-key conjur:user:admin | tr -d '\r')

  docker compose up -d cli
  exec_on cli conjur login -i admin -p "$admin_api_key"
  docker compose cp policy.yml cli:/policy.yml
  exec_on cli conjur policy load -b root -f /policy.yml
  exec_on cli conjur variable set -i secret1 -v "SuperSecret"
  exec_on cli conjur variable set -i secret2 -v "AnotherSecret"

  # Export Pipe API key
  pipe_api_key=$(exec_on conjur conjurctl role retrieve-key conjur:host:pipe | tr -d '\r')
